 ...
 * UserFeedbackSummary
 *
 * Renders a list of feedback categories, each with a pill-style label
 * and a row of dot-separated themes. Pass `highlight: true` on any
 * category to add the star marker (used for the standout category).
 *
 * Usage:
 * <UserFeedbackSummary
 *   title="User feedback"
 *   categories={[
 *     { label: "Navigation", items: ["Too many destinations", "Cluttered hierarchy", "Hard to find things"] },
 *     { label: "Content", items: ["Best journalism", "Deep analysis", "Statsguru", "Rich cricket coverage"], highlight: true },
 *   ]}
 * />
 */
...
export default function UserFeedbackSummary({
  ...
  return (
    <section className="w-full max-w-xl bg-[#fafaf9] px-8 py-8">
      <h2 className="text-lg font-bold tracking-tight text-neutral-900 mb-6">
        {title.toUpperCase()}
      </h2>
        ...
        {categories.map((category) => (
          <div key={category.label}>
            <div className="flex items-center gap-2 mb-2">
              <span className="inline-block rounded bg-neutral-200/70 px-2.5 py-1 font-mono text-xs tracking-wide text-neutral-800">
                {category.label.toUpperCase()}
              </span>
              {category.highlight && (
                <StarIcon className="w-4 h-4 text-neutral-900" />
              )}
            </div>
