...
import { NavCaseStudy, SectionLabel, ClosingSection, ExploreMore, ImpactPanel, impactGrid, impactCell, } from "@/components/ds";
...
const Figure = ({ src, caption, ratio }) => (<figure className="w-full">
    <img src={src} alt={caption ?? ""} loading="lazy" className={`w-full h-auto object-contain ${radius.cardSm}`} style={ratio ? { aspectRatio: ratio } : undefined}/>
    {caption && <figcaption className={`${t.labelSm} text-muted-foreground mt-4 text-center`}>{caption}</figcaption>}
  </figure>);
const Bullet = ({ children }) => (<li className="flex gap-3 items-start">
...
const TradeOff = ({ children }) => (<div className={`${radius.cardSm} border border-dashed border-primary/60 p-5 md:p-6`}>
    <p className={`${t.labelSm} text-primary mb-2.5`}>TRADE-OFF</p>
    <p className="text-sm leading-relaxed text-foreground">{children}</p>
  </div>);
const PersonaCard = ({ name, role, meta, behaviour, pain, opportunity, imageUrl, }) => (<motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fade} className={`${radius.cardSm} border border-border bg-card p-6 md:p-8 max-w-md`}>
...
const Block = ({ eyebrow, title, isFirst, children, }) => (<motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.15 }} variants={fade} className={`${isFirst ? "pt-0" : "pt-12 md:pt-14"} border-b border-border last:border-b-0 `}>
    ...
    return (<div className="min-h-screen bg-background text-foreground">
      ...
      <section className={`pt-24 md:pt-28 pb-8 ${layout.px}`}>
        <div className={`max-w-5xl w-full mx-auto text-center`}>
          <motion.div initial="hidden" animate="visible" variants={fade} className={`flex items-center justify-center gap-4 ${t.labelSm} text-muted-foreground mb-6 md:mb-6`}>
          ...
          <motion.h1 initial="hidden" animate="visible" variants={fade} custom={1} className="text-[2.5rem] sm:text-5xl md:text-6xl lg:text-[4.5rem] font-semibold leading-[1.08] tracking-[0.015em]" style={{ fontFamily: t.displayFont }}>
          ...
          <motion.div initial="hidden" animate="visible" variants={fade} custom={2} className="flex flex-wrap gap-3 justify-center mt-10 md:mt-12">
            {["Lead Product Designer", "~6 months", "iOS & Android"].map((chip) => (<span key={chip} className={`${t.labelSm} px-4 py-2 rounded-full border border-border bg-secondary text-muted-foreground`}>
                {chip}
              </span>))}
          </motion.div>
        ...
        >
          ...
          <motion.p initial="hidden" animate="visible" variants={fade} custom={4} className={`${t.bodyLg} text-muted-foreground max-w-2xl mx-auto mt-12 md:mt-12`}>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fade} className="max-w-5xl mx-auto mt-16 md:mt-20">
            <img src={beforeAfterAsset} alt="Match page before and after redesign, annotated with cognitive load issues and improvements" className="w-full h-auto rounded-lg" loading="lazy"/>
          </motion.div>
        </div>
      </section>
      ...
      <section className={`${spacing.sectionGap} ${layout.px} border-t border-border`}>
        <div className={`${layout.container} max-w-3xl mx-auto`}>
          ...
          <div>
            <img src={problemOverviewAsset} alt="Four match page screens annotated with information overload, data overload, hard to discover and difficult to scan issues" className="w-full h-auto rounded-lg" loading="lazy"/>
          </div>
          ...
          <div className="max-w-4xl mx-auto mt-16 md:mt-20 pt-12 border-t border-border">
            ...
            {
                quote: "Cricinfo used to be simple. Now I have to scroll through 12 options just to find who is bowling.",
                source: "App Store review",
            },
        ].map((q) => (<div key={q.source} className="relative rounded-xl border border-border/60 bg-white p-6 pt-8 shadow-sm">
                  ...
                  <cite className={`${t.labelSm} text-muted-foreground mt-4 pt-3 border-t border-border block not-italic`}>
                    {q.source}
                  </cite>
                </div>))}
            </div>
          ...
          <div className="max-w-4xl mx-auto mt-16 md:mt-20 pt-12 border-t border-border">
        ...
        ].map(([feature, a, b, accent]) => (<tr key={feature} className="border-t border-border">
            ...
            <div className="max-w-4xl mx-auto mt-16 md:mt-20 pt-12 border-t border-border">
              ...
              <div className="max-w-3xl mx-auto grid md:grid-cols-2 grid-cols-2 gap-6">
                <PersonaCard name="Rohan Sharma" role="Live Match Follower" meta="29 · Mumbai, India" behaviour="Visits repeatedly during live games" pain="Too much information to process quickly" opportunity="Surface what matters most" imageUrl={userPersonaAsset.url}/>
                <PersonaCard name="Karan Patel" role="Content Consumer" meta="32 · Delhi, India" behaviour="Reads previews and analysis" pain="Relevant content is hard to discover" opportunity="Improve discovery" imageUrl={userPersonaAsset.url}/>
                <PersonaCard name="Aryan Iyer" role="Casual Fan" meta="24 · Bengaluru, India" behaviour="Visits during major events" pain="Experience can feel overwhelming" opportunity="Simplify and habit forming" imageUrl={userPersonaAsset.url}/>
              </div>
              <div className="mt-10">
                <blockquote className={`pl-5 ${surface.accentLine}`}>
                  <p className="text-lg leading-relaxed">
      ...
      <section className={`${spacing.sectionGap} ${layout.px} border-t border-border`}>
        <div className={`${layout.container} mx-auto`}>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fade} className="grid lg:grid-cols-2 gap-10 items-start">
            <div>
              <p className={`${t.bodyLg} text-muted-foreground mt-5`}>
      ...
      <section className={`${spacing.sectionGap} ${layout.px} border-t border-border`}>
        <div className={`${layout.container} mx-auto`}>
          <div className="max-w-5xl pb-0 mb-0">
            ...
            <div className="flex items-center gap-3 font-mono text-[0.6875rem] tracking-[0.12em] uppercase mb-0 md:mb-0 max-w-5xl mx-auto text-primary" style={{ opacity: 1, transform: "none" }}>
          ...
          <Block eyebrow="Decision 1 — Reduce the tab count" title="Fewer tabs, based on usage, not habit">
            ...
            <div className="max-w-3xl mt-16 md:mt-8 mb-20 pt-4">
              <img src={tabComparisonAsset} alt="Tab bar comparison: 14 tabs before, reduced to 8 after" className="w-full h-auto" loading="lazy"/>
            </div>
          </Block>
          ...
          <Block eyebrow="Decision 3 — Surface a preview" title="See what’s available without leaving the page">
            ...
            <div className="max-w-3xl grid md:grid-cols-2 gap-8 md:gap-10 mb-20 mt-12">
              <img src={liveContentAsset} alt="Live tab surfacing recent commentary preview" className="w-full h-auto" loading="lazy"/>
              <img src={summaryContentAsset} alt="Summary tab surfacing videos and report previews" className="w-full h-auto" loading="lazy"/>
            </div>
          </Block>
          ...
          <Block eyebrow="DECISION 4 — SURFACE CONTENT WHERE IT MATTERS" title="Media, right where you need it">
            ...
            <div className="max-w-3xl grid md:grid-cols-2 gap-8 md:gap-10 mb-20 mt-12">
              <img src={scorecardMediaAsset} alt="Scorecard tab with inline videos and photos" className="w-full h-auto" loading="lazy"/>
              <img src={commsMediaAsset} alt="Commentary tab with inline videos and photos" className="w-full h-auto" loading="lazy"/>
            </div>
          </Block>
          ...
          <Block eyebrow="DECISION 5 — MAKE IT EASY TO MOVE BETWEEN MATCHES" title="Two dropdowns replace multiple actions">
            ...
            <div className="grid md:grid-cols-5 gap-8 md:gap-6 mt-12 mb-20 items-start">
              <Figure src={`${IMG}/84ade12a4cf127b2449f33fb67278fa4fc671e8a?placeholderIfAbsent=true`} caption="Before"/>
          ...
          <Block eyebrow="DECISION 7 — ADD PARTICIPATION WITHOUT ADDING CLUTTER" title="Added Polls and ratings, without disrupting the experience">
            <ul className="flex flex-col gap-4 max-w-3xl">
        ...
        ].map((item) => (<div key={item.label} className={`${radius.cardSm} ${surface.card} p-6`}>
                  <p className={`${t.labelSm} text-muted-foreground`}>{item.label}</p>
      ...
      <section className={`${spacing.sectionGap} ${layout.px} border-t border-border`}>
        <div className={`${layout.container} mx-auto`}>
          ...
          <motion.h2 initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fade} className={`${t.h2} max-w-3xl`} style={{ fontFamily: t.displayFont }}>
        ...
        ].map((s) => (<div key={s.label} className={`${impactCell} p-6 md:p-8`}>
                  <p className="text-3xl md:text-4xl leading-none text-background" style={{ fontFamily: t.displayFont }}>
                    {s.value}
                  </p>
      ...
      {/* ── CLOSE ──────────────────────────── */}
      <ClosingSection quote="The easy fix was to add more. But the users and the data pointed to a different solution: structure that could handle everything." meta="Match Page Redesign — Cricinfo · Prerna Chhajer, Lead Product Designer"/>
